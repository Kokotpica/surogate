...
RECIPE="fp8-hybrid" # you can change this to other recipes like nvfp4, fp8-hybrid
QLORA_BITS="fp4" # you can change this to other quantization bits like fp8, fp4
...
./csrc/build/train --model $MODEL --train-file=$TRAIN_FILES --eval-file=$EVAL_FILE  \
    ...
    --warmup=10 --eval-every-n-steps=0 \
    --qlora-$QLORA_BITS --save --out-dir=qlora_fp4_fp8_cutlass --recompute-block --offload-residual --fp4-backend=cutlass