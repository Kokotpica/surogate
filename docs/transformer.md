...
Each transformer block consists of two sub-blocks: an attention sub-block and an MLP (feed-forward) sub-block:
...
This differs from the original transformer (post-norm) where normalization is applied after residual addition.
...
| **QKV Projection** | Projects input to Query, Key, Value vectors for attention                  |
| **RoPE**           | Rotary Position Embedding - encodes position information directly into Q/K |
| **FlashAttention** | Memory-efficient attention using tiling and recomputation                  |
| **SwiGLU**         | Gated activation: `swish(gate) * up` - better than ReLU/GELU               |
...
```
1. Token Embedding
   │  TokenIds (B, T) → Hidden States (B, T, C)
   ▼
2. Transformer Blocks (repeated L times)
   │  For each layer l in [0, L):
   │    a. LN1 (with fused residual add for l > 0)
   │    b. QKV Projection
   │    c. Optional Q/K Head RMSNorm (Qwen3-style)
   │    d. RoPE (Rotary Position Embedding)
   │    e. FlashAttention
   │    f. Output Projection
   │    g. Fused Residual + LN2
   │    h. MLP Up (gate + up fused)
   │    i. SwiGLU Activation
   │    j. MLP Down
   ▼
3. Final LayerNorm (LNF)
   ▼
4. LM Head (chunked for memory efficiency)
   │  Hidden States → Vocabulary Logits
   ▼
5. Loss Computation
   │  Cross-Entropy with label smoothing (optional)
   ▼
Output: Loss scalar + Accuracy metrics
```

### Activation Storage

During the forward pass, certain activations must be stored for the backward pass:

| Activation | Shape        | Purpose                                         |
| ---------- | ------------ | ----------------------------------------------- |
| `residual` | (B, T, C)    | Input to each block for gradient computation    |
| `ln1`      | (B, T, C)    | RMSNorm output for attention backward           |
| `ln2`      | (B, T, C)    | RMSNorm output for MLP backward                 |
| `qkv`      | (B, T, 3\*C) | Query/Key/Value for attention backward          |
| `att_out`  | (B, T, C)    | Attention output for output projection backward |
| `mlp_up`   | (B, T, 4\*C) | MLP up output for SwiGLU backward               |
| `swiglu`   | (B, T, 2\*C) | SwiGLU output for MLP down backward             |
...
1. Backward through LM Head
   │  d_logits → d_hidden (chunked, fused with forward)
   ▼
2. Backward through Final LayerNorm
   │  Compute weight gradients and propagate to residual
   ▼
3. Transformer Blocks (reverse order, L-1 to 0)
   │  For each layer l:
   │
   │  a. Recompute activations if needed
   │
   │  b. MLP Backward:
   │     - d_mlp_down → d_swiglu
   │     - d_swiglu → d_mlp_up (through SwiGLU backward)
   │     - d_mlp_up → d_ln2
   │     - Accumulate: d_mlp_up_weight, d_mlp_down_weight
   │
   │  c. LN2 Backward (fused with residual)
   │
   │  d. Attention Backward:
   │     - d_out_proj → d_att
   │     - FlashAttention backward → d_qkv
   │     - d_qkv → d_ln1
   │     - Backward RoPE (if needed)
   │     - Accumulate: d_qkv_weight, d_out_weight
   │
   │  e. LN1 Backward
   │
   │  f. Propagate gradient to previous layer's residual
   ▼
4. Backward through Embedding
   │  Accumulate d_embedding_weight
   ▼
5. Gradient Reduction (for distributed training)
   │  AllReduce gradients across ranks (NCCL)
   ▼
Output: All weight gradients ready for optimizer
```
...
When using LoRA with `recompute_lora: true`, an additional optimization is available. Instead of storing LN1/LN2 activations for all layers, they are recomputed on-demand from the residual stream during the LoRA backward pass:

```
# Without recompute_lora: store ln1 for all 36 layers (~360 MB)
# With recompute_lora: recompute ln1 from residual when needed
