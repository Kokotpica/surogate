...
// CPU reference for FP4 block quantization (simplified, no swizzling)
void quantize_fp4_block_cpu(
    std::vector<uint8_t>& out_fp4,
    std::vector<float>& out_block_scales,
    float& out_global_amax,
    const std::vector<float>& in_bf16,
    int M, int K)
{
    ...
    const int scale_rows = div_ceil(M, TILE_SIZE);
    const int scale_cols = div_ceil(div_ceil(K, BLOCK_SIZE), 4) * 4;

    out_fp4.resize(packed_size, 0);
    out_block_scales.resize(scale_rows * scale_cols, 1.0f);

    // Find global amax
    out_global_amax = 0.0f;
    for (const auto& v : in_bf16) {
        out_global_amax = std::max(out_global_amax, std::abs(v));
    }

    // Global encode scale: amax -> FP4_MAX range
    float global_encode_scale = (out_global_amax > 0.0f) ? (FP4_E2M1_MAX / out_global_amax) : 1.0f;

    // Process each row and its 16-element blocks
    for (int row = 0; row < M; ++row) {
        int tile_row = row / TILE_SIZE;
        for (int block_col = 0; block_col < div_ceil(K, BLOCK_SIZE); ++block_col) {
            int col_start = block_col * BLOCK_SIZE;
            int col_end = std::min(col_start + BLOCK_SIZE, K);

            // Find block amax
            float block_amax = 0.0f;
            for (int c = col_start; c < col_end; ++c) {
                float scaled = in_bf16[row * K + c] * global_encode_scale;
                block_amax = std::max(block_amax, std::abs(scaled));
            }

            // Compute block scale (FP8 E4M3)
            float block_scale = (block_amax > 0.0f) ? (block_amax / FP4_E2M1_MAX) : 1.0f;

            // Store block scale (simplified - no swizzling in CPU reference)
            int scale_idx = tile_row * scale_cols + block_col;
            if (scale_idx < static_cast<int>(out_block_scales.size())) {
                out_block_scales[scale_idx] = block_scale;
            }

            // Quantize values in this block
            float inv_block_scale = (block_scale > 0.0f) ? (1.0f / block_scale) : 1.0f;
            for (int c = col_start; c < col_end; ++c) {
                float scaled = in_bf16[row * K + c] * global_encode_scale * inv_block_scale;
                float quantized = quantize_fp4_e2m1_cpu(scaled);
                ...
                // Convert quantized value to 4-bit code
                uint8_t code = 0;
                float abs_q = std::abs(quantized);
                bool negative = quantized < 0.0f;
                for (int i = 0; i < FP4_NUM_VALUES; ++i) {
                    if (std::abs(abs_q - FP4_E2M1_VALUES[i]) < 0.001f) {
                        code = static_cast<uint8_t>(i);
                        break;
                    }
                }
                if (negative) code |= 0x8;  // Sign bit

                if (nibble == 0) {
                    out_fp4[byte_idx] = (out_fp4[byte_idx] & 0xF0) | code;
                } else {
                    out_fp4[byte_idx] = (out_fp4[byte_idx] & 0x0F) | (code << 4);
                }
            }
        }
    }
...
// CPU reference for FP4 dequantization
void dequantize_fp4_block_cpu(
    std::vector<float>& out_bf16,
    const std::vector<uint8_t>& in_fp4,
    ...
    int M, int K)
{
    ...
    for (int row = 0; row < M; ++row) {
        int tile_row = row / TILE_SIZE;
        for (int c = 0; c < K; ++c) {
            int block_col = c / BLOCK_SIZE;
            int scale_idx = tile_row * scale_cols + block_col;
            float block_scale = (scale_idx < static_cast<int>(block_scales.size()))
                                ? block_scales[scale_idx] : 1.0f;
            ...
            uint8_t packed = in_fp4[byte_idx];
            uint8_t code = (nibble == 0) ? (packed & 0x0F) : ((packed >> 4) & 0x0F);

            bool negative = (code & 0x8) != 0;
            int magnitude_idx = code & 0x7;
            float magnitude = FP4_E2M1_VALUES[magnitude_idx];
            float value = negative ? -magnitude : magnitude;

            // Apply scales
            out_bf16[idx] = value * block_scale * global_decode_scale;
        }
    }
}
