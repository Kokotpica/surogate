...
// Maximum FP8 E4M3 representable value
constexpr float FP8_E4M3_MAX = 448.0f;

// CPU reference for per-block quantization
void quantize_per_block_cpu(
    std::vector<uint8_t>& out_fp8,
    ...
    int M, int K, int block_size)
{
    ...
    out_fp8.resize(M * K);
    out_scales.resize(scale_rows * scale_cols);
    ...
    for (int br = 0; br < scale_rows; ++br) {
        for (int bc = 0; bc < scale_cols; ++bc) {
            ...
            float abs_max = 0.0f;
            const int row_start = br * block_size;
            const int col_start = bc * block_size;
            const int row_end = std::min(row_start + block_size, M);
            const int col_end = std::min(col_start + block_size, K);

            for (int r = row_start; r < row_end; ++r) {
                for (int c = col_start; c < col_end; ++c) {
                    abs_max = std::max(abs_max, std::abs(in_bf16[r * K + c]));
                }
            }

            // Compute scale (inverse scale stored for dequantization)
            float scale = (abs_max > 0.0f) ? (abs_max / FP8_E4M3_MAX) : 1.0f;
            out_scales[br * scale_cols + bc] = scale;

            // Quantize elements in this block
            float inv_scale = (scale > 0.0f) ? (1.0f / scale) : 1.0f;
            for (int r = row_start; r < row_end; ++r) {
                for (int c = col_start; c < col_end; ++c) {
                    float val = in_bf16[r * K + c] * inv_scale;
                    // Clamp to FP8 range
                    val = std::clamp(val, -FP8_E4M3_MAX, FP8_E4M3_MAX);
                    // Convert to FP8 (simplified: just store as uint8 approximation)
                    __nv_fp8_e4m3 fp8_val = __nv_fp8_e4m3(val);
                    std::memcpy(&out_fp8[r * K + c], &fp8_val, 1);
                }
            }
        }
    }
...
// CPU reference for per-block dequantization
void dequantize_per_block_cpu(
    std::vector<float>& out_bf16,
    const std::vector<uint8_t>& in_fp8,
    ...
    int M, int K, int block_size)
{
    ...
    for (int r = 0; r < M; ++r) {
        for (int c = 0; c < K; ++c) {
            ...
            int br = r / block_size;
            int bc = c / block_size;
            float scale = scales[br * scale_cols + bc];

            // Dequantize
            __nv_fp8_e4m3 fp8_val;
            std::memcpy(&fp8_val, &in_fp8[r * K + c], 1);
            float val = static_cast<float>(fp8_val);
            out_bf16[r * K + c] = val * scale;
        }
    }
}
