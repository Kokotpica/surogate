...
class SurogateSFT(TokenizeDatasets):
    ...
    def run(self):
        ...
        # Setup data loaders
        train_files = sorted([str(p) for p in Path().glob(f"{self.config.output_dir}/train-*.bin")])
        eval_files = sorted([str(p) for p in Path().glob(f"{self.config.output_dir}/eval.bin")])
